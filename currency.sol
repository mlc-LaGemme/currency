...
contract _template_ is owned {
  ...
  /* INTERNAL - Set the first super admin and ensure that this account is of the good type and actif.*/
  ...
  function setAccountParams(address _targetAccount, bool _accountStatus, int256 _accountType, int256 _debitLimit, int256 _creditLimit) public {
    ...
    // Prevent changing the Type of an Super Admin (2) account
  ...
  function pledge(address _to, int256 _value)  public {
    if (accountType[msg.sender] < 2 || accountType[msg.sender] == 4 ) revert(); // Check that only Super Admin (2) and Pledge Admin (3) can pledge
